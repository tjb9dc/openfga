...
// blocks until every acquired connection is released, which never happens while
// these bugs hold connections hostage, so a plain ds.Close() would hang test
// teardown.
...
// TestIteratorNextRespectsContextDuringPoolAcquire demonstrates that a tuple
// iterator's lazy query execution ignores context cancellation while waiting
// for a pool connection.
//
// SQLTupleIterator.fetchBuffer calls context.WithoutCancel(ctx) before running
// the query, so the pgxpool.Acquire underneath it blocks with no deadline. When
// the pool is exhausted, Next() blocks indefinitely even though the caller's
// context expired long ago. This is the primitive that turns pool contention
// into the permanent server-wide stall reproduced in
// TestReverseExpandPoolStarvationDeadlock below.
...
// TestReverseExpandPoolStarvationDeadlock reproduces a permanent deadlock
// between reverse expansion (ListObjects) and the connection pool.
//
...
// opens its own iterator, which needs a connection of its own:
//
//   - every child blocks in pgxpool.Acquire, unbounded, because
//     fetchBuffer stripped the context (see the test above);
//   - once breadthLimit children are in flight, the parent blocks in
//     pool.Go, so it can neither finish draining its iterator nor release
//     the connection the children are waiting for.
//
// Nothing in the cycle observes cancellation, so the request deadline never
// unwinds it: the connections stay checked out forever and every subsequent
// request that touches the datastore times out until the server is restarted.
...
//
// A single request against a single-connection pool is enough to reproduce it.
